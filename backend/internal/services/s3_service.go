...
func UploadToS3(key string, fileData []byte, contentType string) (string, error) {
	log.Printf("[UploadToS3] Starting upload - Key: %s, Size: %d bytes, ContentType: %s", key, len(fileData), contentType)
	...
	log.Printf("[UploadToS3] Uploading to bucket: %s", s3Bucket)
	...
	if err != nil {
		log.Printf("[UploadToS3] Error uploading to S3: %v", err)
		return "", err
	}

	// Return public URL
	url := fmt.Sprintf("https://%s.s3.amazonaws.com/%s", s3Bucket, key)
	log.Printf("[UploadToS3] Upload successful. URL: %s", url)
	return url, nil
}
